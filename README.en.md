...
- **OpenClaw**: wrapped by the setup.sh-generated `openclaw-round.sh`. Instead of guessing version numbers, it probes the `agent --help` surface at runtime — with `--session-key` (newest) every round routes via `openclaw agent --session-key "matrix-<task-id>" --message …`; with `--session-id` but no `--session-key` (e.g. 2026.4.x) round one derives the session with `--to "matrix-<task-id>" --json`, archives the parsed sessionId so later rounds continue via `--session-id`, and falls back to re-deriving on stale sessions — no manual intervention either way; with `--to` but no `--session-id` each round uses `--to` (same destination derives the same session, best effort, no resume guarantee); missing all three exits with error 99. Startup diagnostics such as `[plugins]` plugin-audit lines are filtered out before results are written back (the root fix is trusting plugins explicitly via `plugins.allow` in openclaw.json)
