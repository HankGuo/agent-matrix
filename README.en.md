...
> - **OpenClaw**: `setup.sh` probes `openclaw agent --help` at runtime and adapts to whatever flags are available — no fixed version number required. Three tiers: has `--session-key` (newest, best experience, direct key-based session routing) → has `--session-id` but no `--session-key` (first round uses `--to` + `--json` to derive session, subsequent rounds `--session-id` resume) → has `--to` but no `--session-id` (each round uses `--to`, same destination derives same session, best effort). **Missing all three (no `--session-key`, `--session-id`, or `--to`) is treated as too old** — `setup.sh` exits with error 99; upgrade OpenClaw and re-run setup.
> - **Hermes Agent**: `setup.sh` calls `hermes chat -q --quiet --resume <sid>` to continue sessions — **there is no probe + graceful fallback like OpenClaw has**. Requires Hermes to support the `--resume` flag (needed for multi-round task context continuity). If `--resume` is unsupported, round one executes normally but from round two onward every round becomes an independent new session with no context carry-over. Either upgrade Hermes Agent or set `AM_RUN_TASK` in the config to fully customize the execution command.
